...
The version is stamped into the plugin DLL and shown beside the DustFinder name in the plugin window. The result is `dist\DustFinder-0.1.2.zip` with this HDT-compatible layout:
...
2. Drag `DustFinder-0.1.2.zip` into the plugin list, or extract its `DustFinder` folder under `%APPDATA%\HearthstoneDeckTracker\Plugins`.
